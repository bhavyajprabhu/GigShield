      ...
      <nav style={{padding:"6px 12px", display:"flex", flexDirection:"column", gap:2}}>
        {NAV.map(n=>(
          <button key={n.id} onClick={()=>{setPage(n.id); setNavOpen(false);}}
            style={{display:"flex", alignItems:"center", gap:11, padding:"10px 12px", borderRadius:9, border:"none",
        ...
        <div style={{flex:1, overflowY:"auto", padding:20, display:"flex", flexDirection:"column", gap:14}}>
          {messages.map((m,i)=>(
            <div key={i} style={{alignSelf: m.role==="ai"?"flex-start":"flex-end", maxWidth:"78%"}}>
              <div style={{padding:"11px 14px", borderRadius:12, fontSize:13.5, lineHeight:1.55,
                background: m.role==="ai" ? "var(--panel-2)" : "var(--amber)",
                color: m.role==="ai" ? "var(--text)" : "#1A1200"}}>{m.text}</div>
            </div>
          ))}
          <div ref={endRef}/>
        </div>
    ...
    <div className="fade-up">
      ...
      <div style={{display:"grid", gridTemplateColumns:"280px 1fr", gap:18}}>
        <div className="card" style={{padding:14, maxHeight:520, overflowY:"auto"}}>
          ...
          {flagged.map(j=>(
            <button key={j.id} onClick={()=>setSelected(j.id)} style={{display:"block", width:"100%", textAlign:"left", padding:"10px 12px", borderRadius:9, border:"none", marginBottom:4,
